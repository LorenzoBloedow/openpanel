...
// The secret declarations and the generic helpers they drive live in
// ./secrets so the Cloudflare build (registry.workerd.ts) can keep them
// without bundling any delivery code.
export * from './secrets';
...
export interface IServerIntegration<T extends IIntegrationConfig['type']> {
  ...
  /** This type's entry in INTEGRATION_SECRET_FIELDS (see ./secrets). */
  secretFields?: readonly IConfigSecret<ConfigOf<T>>[];
}

const slackServer: IServerIntegration<'slack'> = {
  type: 'slack',
  secretFields: INTEGRATION_SECRET_FIELDS.slack,
  notification: {
...
const webhookServer: IServerIntegration<'webhook'> = {
  type: 'webhook',
  secretFields: INTEGRATION_SECRET_FIELDS.webhook,
...
const s3Server: IServerIntegration<'s3_export'> = {
  ...
  secretFields: INTEGRATION_SECRET_FIELDS.s3_export,
};
...
const gcsServer: IServerIntegration<'gcs_export'> = {
  ...
  secretFields: INTEGRATION_SECRET_FIELDS.gcs_export,
};
