 ...
 * reaper until EVERYTHING has fully drained — every session_end emitted, no
 * live session left — and reconciles the Postgres counts. Exits 0 only when
...
const ipForSession = (i: number) =>
  `100.${Math.floor(runId / 256) % 256}.${Math.floor(i / 256) % 256}.${i % 256}`;
...
async function settle(sessions: Session[]) {
  scenario('settle: all sessions opened in Postgres');
  ...
  check('postgres: one session_start per session', starts === ids.length, `got ${starts}`);
}

async function countLive(deviceIds: string[]) {
  const rows = await query<{ c: number }>(
    'SELECT count(*)::int AS c FROM analytics.live_sessions WHERE project_id = $1 AND device_id = ANY($2::text[])',
    [PROJECT_ID, deviceIds],
  );
  return rows[0]?.c ?? 0;
}

async function drain(sessions: Session[]) {
  scenario('drain: reap until nothing remains');
  ...
  while (Date.now() < deadline) {
    ...
    remaining = await countLive(deviceIds);
    console.log(`   …session_end ${ends}/${target}, live sessions remaining ${remaining}`);
  ...
  check('postgres: one session_end per session', ends === target, `got ${ends}`);
  check('postgres: live sessions fully drained', remaining === 0, `${remaining} left`);
}

async function reconcile(sessions: Session[]) {
  scenario('reconcile: Postgres event counts');
  const ids = sessions.map((s) => s.sessionId);
  const n = ids.length;
  const rows = await query<{ name: string; c: number }>(
    `SELECT name, count(*)::int AS c FROM analytics.events
     WHERE project_id = $1 AND session_id = ANY($2::text[])
     GROUP BY name ORDER BY c DESC`,
    [PROJECT_ID, ids],
  );
  const byName = new Map(rows.map((r) => [r.name, r.c]));
